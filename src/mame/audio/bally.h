...
#define MCFG_BALLY_CHEAP_SQUEAK_SOUND_ACK_W(_devcb) \
	devcb = &downcast<bally_cheap_squeak_device &>(*device).set_sound_ack_w_callback(DEVCB_##_devcb);
...
public:
	...
	template <class Object> devcb_base &set_sound_ack_w_callback(Object &&cb) { return m_sound_ack_w_cb.set_callback(std::forward<Object>(cb)); }
	...
	void cheap_squeak_io_map(address_map &map);
...
private:
	...
	devcb_write_line m_sound_ack_w_cb;
